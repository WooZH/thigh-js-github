...
const toSpecifiedDenomination = {
  ...
  GWEI: (bigNumber: any) => bigNumber.times(BIG_NUMBER_GWEI_MULTIPLIER).round(9),
...
const addCurrencies = (a: any, b: any, options = {}) => {
  const { aBase, bBase, ...conversionOptions } = options as any;
...
const subtractCurrencies = (a: any, b: any, options: any = {}) => {
...
const multiplyCurrencies = (a: any, b: any, options: any = {}) => {
...
const conversionGreaterThan = ({ ...firstProps }, { ...secondProps }) => {
  const firstValue = converter({ ...firstProps } as Input);
  const secondValue = converter({ ...secondProps } as Input);
